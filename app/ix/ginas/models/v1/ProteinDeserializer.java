...
public class ProteinDeserializer extends JsonDeserializer<Protein> {
    ...
    public ProteinDeserializer () {
        
    }

    public Protein deserialize (JsonParser parser, DeserializationContext ctx)
        throws IOException, JsonProcessingException {
        ctx.getConfig().withHandler(handler);
        JsonNode tree = parser.getCodec().readTree(parser);
        Protein struc = parser.getCodec().treeToValue(tree, Protein.class);
        return struc;
    }
}
